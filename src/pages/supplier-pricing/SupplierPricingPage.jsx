...
const PricingModal = ({ catalog, suppliers, onClose, onSave, saving }) => {
  const [form, setForm] = useState({
    ...
    generic_name:   catalog?.generic_name ?? "",
    effective_date: catalog?.effective_date ?? "",
    notes:          catalog?.notes         ?? "",
    status:         catalog?.status        ?? "active",
    tiers:          catalog?.tiers?.length
      ? catalog.tiers.map((t) => ({ tier_label: t.tier_label, price: t.price }))
      : [{ tier_label: "", price: "" }],
  });
  ...
  const addTier = () =>
    setForm((f) => ({ ...f, tiers: [...f.tiers, { tier_label: "", price: "" }] }));
                ...
                {form.tiers.map((tier, i) => (
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => setTier(i, "tier_label", e.target.value)}
                      required
                      placeholder="e.g. 1-2vls"
...
const CatalogRow = ({ catalog, canManage, onEdit, onDelete }) => {
    ...
    <>
      ...
      >
        ...
        <td className="px-4 py-3">
          ...
          <p className="text-xs text-gray-500">{catalog.generic_name}</p>
        </td>
...
export default function SupplierPricingPage() {
  ...
  const canManage = hasPermission("manage_supplier_pricing");
  ...
  // Fetch all active suppliers for filter/form dropdowns
  ...
  // Fetch price catalogs
  const { data, isLoading } = useQuery({
    queryKey: ["supplier-price-catalogs", search, supplierFilter, page],
    queryFn: async () => {
      const res = await api.get("/supplier-price-catalogs", {
  ...
  const createMutation = useMutation({
    mutationFn: (payload) => api.post("/supplier-price-catalogs", payload),
    onSuccess: () => {
      queryClient.invalidateQueries(["supplier-price-catalogs"]);
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => api.put(`/supplier-price-catalogs/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries(["supplier-price-catalogs"]);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/supplier-price-catalogs/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries(["supplier-price-catalogs"]);
  ...
  const importMutation = useMutation({
    mutationFn: ({ file, supplier_id }) => {
      ...
      return api.post("/supplier-price-catalogs/import", fd, {
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={7} className="text-center py-12">
                  ...
                  <p className="text-gray-400">No price entries found</p>
                </td>
              </tr>
            ) : (
              catalogs.map((catalog) => (
                <CatalogRow
                  ...
                  catalog={catalog}
                  canManage={canManage}
                  onEdit={(c) => { setSelected(c); setShowModal(true); }}
